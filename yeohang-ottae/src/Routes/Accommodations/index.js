...
const useLoad = (sightX, sightY) => {
    ...
    const sendToRestaurantServer = async (sightX, sightY) => {
        ...
        await axios.post('/accommodation', sightXY)
            .then((res) => {
    ...
    const LoadRestaurant = async () => {
        try {
            const results = await axios.get('/accommodation');
            ...
            for (let i = 1; i <= size; i++) {
                ...
                if (i == randomNum) {
                    ...
                    otherAccommodation.push(results.data[i - 1]);
                    count = count + 1;
                }
            }
            setOthers(otherAccommodation);
        } catch {
            console.log("err");
        } finally {
            setLoading(false);
                    ...
                    <Content>
                        <>
                            ...
                            <Data>
                                ...
                                {/* <Link to="/Accommodation"><button>주변 숙박시설 추천</button></Link> */}
